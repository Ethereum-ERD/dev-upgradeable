...
contract('Gas cost tests', async accounts => {
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 ether, withdraw 100 EUSD
  ...
  it("", async () => {
    ...
    //50 accts open Trove with 1 ether and withdraw 100 EUSD
  ...
  it("", async () => {
    ...
    //60 accts open Trove with 1 ether and withdraw 100 EUSD
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 15 ether, withdraw 100 EUSD
    await th.openTrove_allAccounts(accounts.slice(101, 111), contracts, dec(15, 'ether'), dec(100, 18))

    //65 accts open Trove with 1 ether and withdraw 100 EUSD
  ...
  it("", async () => {
    ...
    await borrowerOperations.openTrove(_100pct, dec(1, 27), accounts[999],ZERO_ADDRESS, { from: accounts[999], value: dec(1, 27) })
    await stabilityPool.provideToSP(dec(1, 27), ZERO_ADDRESS, { from: accounts[999] })
    assert.equal((await stabilityPool.getTotalEUSDDeposits()), dec(1, 27))
