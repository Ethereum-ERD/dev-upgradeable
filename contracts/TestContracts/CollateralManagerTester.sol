...
contract CollateralManagerTester is
    OwnableUpgradeable,
    ...
    ICollateralManager
{
    ...
    ) public view returns (uint256 totalValue, uint256[] memory values) {
    ...
    function computeICR(
        ...
        (uint256 totalValue, ) = getValues(_tokens, _amounts);
    ...
    function getCurrentICR(address _borrower) external view returns (uint256) {
        return
            troveManager.getCurrentICR(_borrower, priceFeed.fetchPrice_view());
    }
